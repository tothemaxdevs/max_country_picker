...
import 'package:max_country_list/view/list_picker_modal.dart';
import 'package:max_country_list/view/list_picker_page.dart';
...
class _MaxCountryPickerState extends State<MaxCountryPicker> {
  String? title;
  late MaxCountry initialCountry = MaxCountryList.list[MaxCountryList.list
      .indexWhere((element) => element.code == widget.initialCountryCode)];
  @override
  void initState() {
    super.initState();
  }
  ...
  Future<void> openOption(BuildContext context) async {
    if (widget.viewMode == ViewMode.page) {
      var picker = await Navigator.push(
          context,
          MaterialPageRoute(
      ...
      if (picker is MaxCountry) {
        setState(() {
          initialCountry = picker;
        });
      }
    } else if (widget.viewMode == ViewMode.modal) {
      listPickerModal(
        context,
